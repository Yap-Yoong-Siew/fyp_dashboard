...
// Main Tab Objects
    ...
    if (!LabelCreate(m_LblURL, first_column_start, y, first_column_start + normal_label_width, y + element_height, "m_LblURL", "www.yapyoongsiew.com"))                                          return false;
...
// Order does not matter.
    FileWrite(fh, "stringDate");
    FileWrite(fh, sets.stringDate);
...
bool CAccountProtector::LoadSettingsFromDisk()
{
    ...
    while (!FileIsEnding(fh))
    {
        ...
        if(var_name == "stringDate")
            sets.stringDate = var_content;
...
void CAccountProtector::Logging_Current_Settings()
{
    ...
    Logging("sets.stringDate" + sets.stringDate);
...
{
    double floating_profit = 0;
    double daily_floating_profit = 0;
    int floating_profit_points = 0;
    int daily_floating_profit_points = 0;
    double daily_profit_loss_units = 0, daily_profit_loss_perc = 0;
    double daily_profit_loss_units_without_floating = 0, daily_profit_loss_perc_without_floating = 0;
    double daily_equity_profit_loss_units = 0;
    ...
    for (int i = 0; i < OrdersTotal(); i++)
    {
        ...
        // Calculating daily FLOATING profit/loss if necessary.
    if (
       ...
       )
    {
        ...
        for (int i = 0; i < OrdersTotal(); i++)
        {
            ...
            if (CheckFilterLossProfit(OrderProfit())) continue;
            if (OrderCloseTime() < start_of_today) continue;
            if (CheckFilterSymbol(OrderSymbol())) continue;
            if ((OrderClosePrice() == 0) || (iClose(OrderSymbol(), Period(), 0) == 0) || (SymbolInfoDouble(OrderSymbol(), SYMBOL_ASK) == 0) || (SymbolInfoDouble(OrderSymbol(), SYMBOL_BID) == 0)) return; // Prevents incorrect profit/loss values when working with account that is traded via API calls.
            if (CheckFilterComment(OrderComment())) continue;
            // Starting from -1 index to check for orders irrespective of their Magic numbers.
            for (int j = -1; j < magic_array_counter; j++)
            {
                ...
                daily_floating_profit += OrderProfit();
                if (sets.CountCommSwaps) daily_floating_profit += OrderCommission() + OrderSwap();

                if (OrderType() == OP_BUY) daily_floating_profit_points += (int)MathRound((MarketInfo(OrderSymbol(), MODE_BID) - OrderOpenPrice()) / MarketInfo(OrderSymbol(), MODE_POINT));
                else if (OrderType() == OP_SELL) daily_floating_profit_points += (int)MathRound((OrderOpenPrice() - MarketInfo(OrderSymbol(), MODE_ASK)) / MarketInfo(OrderSymbol(), MODE_POINT));
                break; // Order already processed - no point to process this order with other magic numbers.
            }
        }
       ...
       )
    {
        ...
        for (int i = 0; i < OrdersHistoryTotal(); i++)
        {
            ...
            for (int j = -1; j < magic_array_counter; j++)
            {
                ...
                daily_profit_loss_units_without_floating = daily_profit_loss_units;
    ...
    // Displaying floating profit, account balance, account equity, and current broker time in a chart comment
     
    string comment = "Floating Profit: " + DoubleToString(floating_profit, 2) + " " + AccountInfoString(ACCOUNT_CURRENCY) + "\n" +
                     "Account Balance: " + DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 2) + " " + AccountInfoString(ACCOUNT_CURRENCY) + "\n" +
                     "Account Equity: " + DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 2) + " " + AccountInfoString(ACCOUNT_CURRENCY) + "\n" +
                     "Current Broker Time: " + TimeToString(TimeCurrent(), TIME_DATE | TIME_MINUTES | TIME_SECONDS) + "\n" +
                     "Daily PnL %: " + DoubleToString(daily_profit_loss_perc, 2) + "\n" +
                     "Daily PnL $: " + DoubleToString(daily_profit_loss_units, 2) + "\n" +
                     "Starting balance of the day: " + DoubleToString((AccountInfoDouble(ACCOUNT_BALANCE) - daily_profit_loss_units_without_floating), 2) + "\n" +
                     "Starting equity of the day: " + DoubleToString(sets.SnapEquity, 2);
                    //  "datetime start_of_today: " + start_of_today; 
    Comment(comment);
    // CommentLab("this is a test comment");
...
// Timeout by timer.
    ...
    if (sets.RecaptureSnapshots)
    {
        ...
        if (TerminalInfoInteger(TERMINAL_TRADE_ALLOWED)) 
        {
            // if (!DoNotDisableActions) sets.RecaptureSnapshots = True;
...
void CAccountProtector::Trigger_Actions(string title)
{
    ...
    // Recapture snapshots. // during real trigger, shouldn't recapture snapshots, because it should only be done during rolloever
    // if (sets.RecaptureSnapshots)
    // {
    //     // if (!DoNotDisableActions) sets.RecaptureSnapshots = True;
    //     Logging("ACTION IS TAKEN: Recapture snapshots.");
